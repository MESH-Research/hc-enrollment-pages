function IdPSelectUI() {
    ...
    */
	...
	idpDataPublic = JSON.parse(JSON.stringify(idpData));
        stripPrivate(humanitiesCommonsHiddenIdPs);
    ...
    */
    ...
    var setupLocals = function (paramsSupplied) {
        ...
        cookieDomain = paramsSupplied.cookieDomain;
	...
	// disabled for Humanities Commons to prevent FATAL DISCO error
        //if (!protocolOk) {
    ...
    */
    ...
    /**
       Strips the supplied IdP list from the idpDataPublic
    */
    var stripPrivate = function(privateList) {
    
        if (null == privateList || 0 == privateList.length) {
            return;
        }
        var i;
        var j;
        for (i = 0; i < privateList.length; i++) {
            for (j = 0; j < idpDataPublic.length; j++) {
                if (getEntityId(idpDataPublic[j]) == privateList[i]) {
                    idpDataPublic.splice(j, 1);
                    break;
                }
            }
        }
    ...
    */
    var buildIdPEntryTile = function(parentDiv, preferredTile) {
        ...
        dropDownControl = new TypeAheadControl(idpDataPublic, textInput, hidden, button, maxIdPCharsDropDown, getLocalizedName, getEntityId, geticon, ie6Hack, alwaysShow, maxResults, getKeywords);

/* We'll use type ahead only.
...
*/
    ...
    */
    var buildIdPDropDownListTile = function(parentDiv, preferredTile) {
        ...
        var idp;
        for(var i=0; i<idpDataPublic.length; i++){
            idp = idpDataPublic[i];
    ...
    */
    ...
    var setAutoDispatchCookie = function(days) {
        ...
        document.cookie=autoFollowCookie + '=1; domain=' + cookieDomain + '; path=/; expires=' + expireDate.toUTCString();
    }
    ...
    */
    var saveUserSelectedIdPs = function(idps){
        ...
        document.cookie='_saml_idp' + '=' + cookieData.join('%20') + '; domain=' + cookieDomain + '; path = /' +
            ((expireDate===null) ? '' : '; expires=' + expireDate.toUTCString());
